...
public class OnvifDevice {
	...
	// enable/disable logging of SOAP messages
	...
	/*
	 * @param url is http://host or http://host:port or https://host or
	 * https://host:port
	 * 
	...
	protected void init() throws ConnectException, SOAPException {
		...
		// resetSystemDateAndTime(); // don't modify the camera in a
		// constructor.. :)
		...
		if (capabilities.getMedia() != null && capabilities.getMedia().getXAddr() != null) {
			...
			this.media = getServiceProxy((BindingProvider) media, capabilities.getMedia().getXAddr())
					.create(Media.class);
		}
		...
		if (capabilities.getImaging() != null && capabilities.getImaging().getXAddr() != null) {
			...
			this.imaging = getServiceProxy((BindingProvider) imaging, capabilities.getImaging().getXAddr())
		...
		if (capabilities.getEvents() != null && capabilities.getEvents().getXAddr() != null) {
			...
			this.events = getServiceProxy((BindingProvider) events, capabilities.getEvents().getXAddr())
	...
	public JaxWsProxyFactoryBean getServiceProxy(BindingProvider servicePort, String serviceAddr) {
		...
		if (verbose) {
			// these logging interceptors are depreciated, but should be fine
