 ...
 * @param DPSDK_CORE_ALARM_VEHICLEINFOUPLOAD_CHECKIN      = 217; 车载自定义信息上传
 *                                                        CheckIn
 * @param DPSDK_CORE_ALARM_VEHICLEINFOUPLOAD_CHECKOUT     = 218; 车载自定义信息上传
 *                                                        CheckOut
 * @param DPSDK_CORE_ALARM_MOTOR_END                      = 299;
 * 
 * @param DPSDK_CORE_ALARM_IVS_ALARM_BEGIN                = 300;
 *                                                        智能设备报警类型在dhnetsdk.h基础上+300（DMS服务中添加）
 ...
 * 
 * @param DPSDK_CORE_ALARM_TRAFFIC_RUNREDLIGHT            = 0x00000100 + 300;
 *                                                        交通违章-闯红灯事件
 * @param DPSDK_CORE_ALARM_TRAFFIC_OVERLINE               = 0x00000101 + 300;
 *                                                        交通违章-压车道线事件
 * @param DPSDK_CORE_ALARM_TRAFFIC_RETROGRADE             = 0x00000102 + 300;
 *                                                        交通违章-逆行事件
 * @param DPSDK_CORE_ALARM_TRAFFIC_TURNLEFT               = 0x00000103 + 300;
 *                                                        交通违章-违章左转
 * @param DPSDK_CORE_ALARM_TRAFFIC_TURNRIGHT              = 0x00000104 + 300;
 *                                                        交通违章-违章右转
 * @param DPSDK_CORE_ALARM_TRAFFIC_UTURN                  = 0x00000105 + 300;
 *                                                        交通违章-违章掉头
 * @param DPSDK_CORE_ALARM_TRAFFIC_OVERSPEED              = 0x00000106 + 300;
 *                                                        交通违章-超速
 * @param DPSDK_CORE_ALARM_TRAFFIC_UNDERSPEED             = 0x00000107 + 300;
 *                                                        交通违章-低速
 * @param DPSDK_CORE_ALARM_TRAFFIC_PARKING                = 0x00000108 + 300;
 *                                                        交通违章-违章停车
 * @param DPSDK_CORE_ALARM_TRAFFIC_WRONGROUTE             = 0x00000109 + 300;
 *                                                        交通违章-不按车道行驶
 * @param DPSDK_CORE_ALARM_TRAFFIC_CROSSLANE              = 0x0000010A + 300;
 *                                                        交通违章-违章变道
 * @param DPSDK_CORE_ALARM_TRAFFIC_OVERYELLOWLINE         = 0x0000010B + 300;
 *                                                        交通违章-压黄线
 * @param DPSDK_CORE_ALARM_TRAFFIC_DRIVINGONSHOULDER      = 0x0000010C + 300;
 *                                                        交通违章-路肩行驶事件
 * @param DPSDK_CORE_ALARM_TRAFFIC_YELLOWPLATEINLANE      = 0x0000010E + 300;
 *                                                        交通违章-黄牌车占道事件
 * @param DPSDK_CORE_ALARM_CROSSFENCEDETECTION            = 0x0000011F + 300;
 *                                                        翻越围栏事件
 * @param DPSDK_CORE_ALARM_ELECTROSPARKDETECTION          = 0X00000110 + 300;
 *                                                        电火花事件
 * @param DPSDK_CORE_ALARM_TRAFFIC_NOPASSING              = 0x00000111 + 300;
 *                                                        交通违章-禁止通行事件
 * @param DPSDK_CORE_ALARM_ABNORMALRUNDETECTION           = 0x00000112 + 300;
 *                                                        异常奔跑事件
 * @param DPSDK_CORE_ALARM_RETROGRADEDETECTION            = 0x00000113 + 300;
 *                                                        人员逆行事件
 * @param DPSDK_CORE_ALARM_INREGIONDETECTION              = 0x00000114 + 300;
 *                                                        区域内检测事件
 * @param DPSDK_CORE_ALARM_TAKENAWAYDETECTION             = 0x00000115 + 300;
 *                                                        物品搬移事件
 * @param DPSDK_CORE_ALARM_PARKINGDETECTION               = 0x00000116 + 300;
 *                                                        非法停车事件
 * @param DPSDK_CORE_ALARM_FACERECOGNITION                = 0x00000117 + 300;
 *                                                        人脸识别事件
 * @param DPSDK_CORE_ALARM_TRAFFIC_MANUALSNAP             = 0x00000118 + 300;
 *                                                        交通手动抓拍事件
 * @param DPSDK_CORE_ALARM_TRAFFIC_FLOWSTATE              = 0x00000119 + 300;
 *                                                        交通流量统计事件
 * @param DPSDK_CORE_ALARM_TRAFFIC_STAY                   = 0x0000011A + 300;
 *                                                        交通滞留事件
 * @param DPSDK_CORE_ALARM_TRAFFIC_VEHICLEINROUTE         = 0x0000011B + 300;
 *                                                        有车占道事件
 * @param DPSDK_CORE_ALARM_MOTIONDETECT                   = 0x0000011C + 300;
 *                                                        视频移动侦测事件
 * @param DPSDK_CORE_ALARM_LOCALALARM                     = 0x0000011D + 300;
 *                                                        外部报警事件
 * @param DPSDK_CORE_ALARM_PRISONERRISEDETECTION          = 0X0000011E + 300;
 *                                                        看守所囚犯起身事件
 * @param DPSDK_CORE_ALARM_IVS_B_ALARM_END                = 0X0000011E + 301;
 *                                                        以上报警都为IVS_B服务的报警类型，与SDK配合
 * @param DPSDK_CORE_ALARM_VIDEODIAGNOSIS                 = 0X00000120 + 300;
 *                                                        视频诊断结果事件
 * @param DPSDK_CORE_ALARM_IVS_V_ALARM                    = 0X00000120 + 300;
 * @param DPSDK_CORE_ALARM_IVS_ALARM_END                  = 1000;
 *                                                        智能设备报警类型的范围为300-1000
...
public class dpsdk_alarm_type_e {
	...
	// 开关量，可控，请注意接下来的public static final int
	// DPSDK_CORE_ALARM_SCS_DOOR_SWITCH这个不能作为BEGIN用
